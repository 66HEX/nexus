...
const Pricing = () => {
    ...
    return (
        <section id="pricing" className="bg-black text-white py-12 sm:py-24 px-3 sm:px-4 lg:px-24 relative overflow-hidden">
            <div className="container mx-auto relative">
                <div className="text-center mb-8 sm:mb-16">
                    <div className="inline-flex items-center rounded-full bg-white/[0.05] border border-white/10 px-3 py-1 text-sm text-white/70 backdrop-blur-sm mb-6 sm:mb-8">
                        Pricing
                    </div>
                    <h2 className="text-3xl md:text-5xl font-medium tracking-tight mb-4 sm:mb-6">
                        Choose your
                        <br />
                        <span className="bg-gradient-to-r from-slate-400 to-slate-600 text-transparent bg-clip-text">strength pathway</span>
                    </h2>
                    ...
                    {tiers.map((tier, index) => (
                        <div
                            ...
                            className={`bg-white/[0.05] backdrop-blur-md rounded-2xl lg:rounded-3xl p-6 lg:p-8 border ${
                                tier.highlighted
                                    ? '[background:linear-gradient(45deg,#0D0D0D,#0D0D0D_100%,#0D0D0D)_padding-box,conic-gradient(from_var(--border-angle),theme(colors.slate.600/.48)_80%,_theme(colors.slate.400)_86%,_theme(colors.slate.300)_90%,_theme(colors.slate.400)_94%,_theme(colors.slate.600/.48))_border-box] border border-transparent animate-border shadow-lg'
                                    : 'border-white/[0.05]'
                            } h-fit relative`}
                        >
                            {tier.highlighted && (
                                <div className="absolute -top-4 left-1/2 transform -translate-x-1/2">
                                    <span className="bg-gradient-to-r from-slate-400 to-slate-600 text-white text-sm px-3 py-1 rounded-full">
                                        Most Popular
                                    </span>
                                </div>
                            )}
                            ...
                            <div className="space-y-4">
                                ...
                                <button className={`w-full py-2 px-4 rounded-lg text-sm font-medium transition-colors ${
                                    tier.highlighted
                                        ? 'bg-gradient-to-r from-slate-400 to-slate-600 hover:from-slate-400 hover:to-slate-500 text-white'
